...
@directions = [
  Vector[0,  1], # Right
  Vector[1,  0], # Down
  Vector[1,  1], # Diagonal right
  Vector[1, -1], # Diagonal left
]
...
0.upto(16) do |x|
  0.upto(20) do |y|
    @directions.each do |direction|
      a = (0..3).map { |n| @numbers[*(Vector[y, x] + direction * n)] }.compact
      possibilities << a unless a.empty?
    end
  end
end
puts possibilities.max_by { |a| a.reduce(&:*) }.reduce(&:*)
